...
sub_companies <-  read_csv(paste0(path,"mi_companies_details.csv.xz")) %>%
  filter(`Physical County` %in% area) %>%
...
company_list <- unique(temp$duns)
sub_operation <-  read_csv(paste0(path,"mi_financial_info.csv.xz")) %>%
  select(company_name=`Company Name`,
         duns = `D-U-N-S@ Number`,
         measure,
         value,
         year) %>%
  filter(duns %in% company_list) %>%
...
subva_bg2010 <-  read_csv(paste0(path,"mi_subva_geolocated_bg2010.csv.xz")) 
subva_bg2020 <-  read_csv(paste0(path,"mi_subva_geolocated_bg2020.csv.xz")) 


# merge all the data and save ------------------------------------------------
feature <- merge(temp, temp2, by.x=c('duns','company_name'), by.y=c('duns','company_name')) 
feature_bg2010 <- merge(feature[feature$year<2020,], subva_bg2010, by.x=c('duns','company_name'), by.y=c('duns','company_name')) 
feature_bg2020 <- merge(feature[feature$year>2019,], subva_bg2020, by.x=c('duns','company_name'), by.y=c('duns','company_name')) 
feature_bg <- rbind(feature_bg2010,feature_bg2020)

# save the data
readr::write_csv(feature_bg, xzfile(paste0(path,"mi_subva_features_bg.csv.xz"), compression = 9))
