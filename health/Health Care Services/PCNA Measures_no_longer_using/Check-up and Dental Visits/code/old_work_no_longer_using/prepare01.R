...
source("code/mrp_utils.R")

### get data to fit MRP model ###
va_brfss <- readRDS("/project/biocomplexity/sdad/projects_data/mc/data_commons/dc_health_behavior_diet/brfss/va_2011_clean.rds")
# drop refuse and don't know responses
va_brfss <- va_brfss[!(va_brfss$checkup1 %in% c(7, 9, NA)),]
va_brfss$checkup_binary <- ifelse(va_brfss$checkup1 == 1, 1, 0)
...
# transform to binary
brfss_2019$checkup1 <- replace(brfss_2019$checkup1, brfss_2019$checkup1 == 88, 0)
brfss_2019$checkup_binary <- ifelse(brfss_2019$checkup1 == 1, 1, 0)
...
st_prev_logit <- qlogis(unname(svymean(~checkup_binary, brfss_2019_design)[1]))

### MRP ###
brfss_mrp <- cmdstan_model("code/brfss_mrp2.stan")

# annual check up #
data_list <- list(N = nrow(va_brfss), 
                       y = va_brfss$checkup_binary,
                       J = ncol(ps_mat), 
...
tract_res <- data.frame(tract_code = rownames(ps_mat),
                             mean_check = res$mean,
                             median_check = res$median,
                             sd_check = res$sd,
                             mad_check = res$mad,
                             lower_pi = ci[,1],
                             upper_pi = ci[,2])

saveRDS(tract_res, "data/working/check_2019_pred_tr.rds")
...
va_ct_est <- aggregate_tract_est %>% group_by(county_geoid) %>%
  summarize(prop_check = sum(mean_check * total_18_plus_pop,  na.rm = TRUE)/sum(total_18_plus_pop, na.rm = T))
...
out_tr <- left_join(tract_res[c("tract_code","mean_check")], va_tracts, by = c("tract_code" = "geoid"))
out_ct <- left_join(va_ct_est, va_counties, by = c("county_geoid" = "geoid"))

# add year
out_tr["year"] <- 2019
out_tr["measure"] <- "perc_annual_checkup"
out_tr["measure_type"] <- "percent"
out_tr$mean_check <- out_tr$mean_check * 100
...
# add year
out_ct["year"] <- 2019
out_ct["measure"] <- "perc_annual_checkup"
...
#write_csv(out_tr, "data/distribution/va_tr_brfss_2019_percent_annual_checkup.csv")
#write_csv(out_ct, "data/distribution/va_ct_brfss_2019_percent_annual_checkup.csv")
