...
population$providers_2sfca <- catchment_ratio(
  ...
  consumers_value = "population", providers_id = "ID", providers_value = "beds", verbose = TRUE
) * 1000

pal <- colorBin("RdYlBu", population$providers_2sfca)

plot_2sfca <- map %>%
  addControl("Hospital Beds Per 1,000 People (2-Step Floating Catchment Area)", "topright") %>%
...
plot_e2sfca <- map %>%
  addControl("Hospital Beds Per 1,000 People (enhanced 2-Step Floating Catchment Area)", "topright") %>%
...
plot_3sfca <- map %>%
  addControl("Hospital Beds Per 1,000 People (3-Step Floating Catchment Area)", "topright") %>%
...
# NOTE - I believe hosp_cnt should stay hosp_cnt and not be changed to hosp_beds_cnt.  This has been corrected as the measure name in the va file.
```
...
write.csv(va_bg_2021_access_scores_hosp, "va_hdcttrbg_hifld_2021_access_scores_hospitals", row.names = FALSE )
...
sf::st_write(
  va_bg_2021_access_scores_hosp, con,
  c("dc_health_behavior_diet", "va_hdcttrbg_hifld_2021_access_scores_hospitals"),
