...
access_care_2021 = read_csv("/Users/michaelvaden/Downloads/access_care_indicator_geo20.csv") %>% mutate(geoid = as.character(geoid)) %>% filter(year == 2021) %>% dplyr::select(c(geoid, value))
```
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2021$Incarceration))
...
```{r}
inc_2021_predicted = predict(predict_incarceration, hoi_recreated_normalized_2021 %>% dplyr::select(-c(geoid)))

inc_2021_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2021 %>% dplyr::select(-c(geoid)))
...
table(HOI_data_2021_quintile$value)

mean(HOI_data_2021_quintile$value)
...
```{r}
#write.csv(indicators_2021, "indicators_used_for_LE/indicators_2021.csv")
```
