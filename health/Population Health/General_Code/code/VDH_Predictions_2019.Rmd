...
access_care_2019 = read_csv("/Users/michaelvaden/Downloads/access_care_indicator_geo20.csv") %>% mutate(geoid = as.character(geoid)) %>% filter(year == 2019) %>% dplyr::select(c(geoid, value))
```
...
```{r}
list_2019 = list(access_care_2019, average_years_schooling_2019, employment_access_2019, labor_force_2019, population_density_2019, walkability_2021, segregation_2019, income_inequality_2019, affordability_2019, environment_hazard_2022, access_to_food_new_2017, material_deprivation_indicator_2019, incarceration %>% filter(year == 2019), mobility_2019)
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2019$Incarceration))
...
```{r}
inc_2019_predicted = predict(predict_incarceration, hoi_recreated_normalized_2019 %>% dplyr::select(-c(geoid)))

inc_2019_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2019 %>% dplyr::select(-c(geoid)))
...
table(HOI_data_2019_quintile$value)

mean(HOI_data_2019_quintile$value)
...
```{r}
#write.csv(indicators_2019, "indicators_used_for_LE/indicators_2019.csv")
```
