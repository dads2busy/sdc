...
# for access to care we need to use 2017 data
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2016$Incarceration))
...
```{r}
inc_2016_predicted = predict(predict_incarceration, hoi_recreated_normalized_2016 %>% dplyr::select(-c(geoid)))

inc_2016_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2016 %>% dplyr::select(-c(geoid)))
...
table(HOI_data_2016_quintile$value)

mean(HOI_data_2016_quintile$value)
