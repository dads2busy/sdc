...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2017$Incarceration))
...
```{r}
inc_2017_predicted = predict(predict_incarceration, hoi_recreated_normalized_2017 %>% dplyr::select(-c(geoid)))

inc_2017_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2017 %>% dplyr::select(-c(geoid)))
...
table(HOI_data_2017_quintile$value)

mean(HOI_data_2017_quintile$value)
...
```{r}
#write.csv(indicators_2017, "indicators_used_for_LE/indicators_2017.csv")
```
