...
# for access to care we need to use 2017 data for 2015
access_care_2017 = read_csv("/Users/michaelvaden/Downloads/access_care_indicator_geo20.csv") %>% mutate(geoid = as.character(geoid)) %>% filter(year == 2017) %>% dplyr::select(c(geoid, value))
```
...
```{r}
hoi_recreated_normalized_2015 <- hoi_recreated_imputed_2015 %>% mutate(across(where(is.numeric), scale_numeric)) %>% mutate_if(is.numeric, round, digits=3)
...
```{r}
predict_incarceration = lm(Incarceration~., data=data_matrix %>% dplyr::select(-c(`LifeExpectancy`, `Incarceration`)) %>% mutate(Incarceration = hoi_recreated_imputed_2015$Incarceration))
...
```{r}
inc_2015_predicted = predict(predict_incarceration, hoi_recreated_normalized_2015 %>% dplyr::select(-c(geoid)))

inc_2015_predicted_normalized = predict(predict_incarceration_normalized, hoi_recreated_normalized_2015 %>% dplyr::select(-c(geoid)))
...
predict_LE_model = lm(LifeExpectancy~., data=X1 %>% dplyr::select(-c(HOI)))
...
mean(HOI_data_2015_quintile$value)
...
```{r}
#write.csv(indicators_2015, "indicators_used_for_LE/indicators_2015.csv")
```
