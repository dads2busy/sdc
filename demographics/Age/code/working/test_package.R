...
for (vyear in yearlist){
  ...
  subset_dmg <- merge(subset_dmg, fairfax_pc_geo, by='parid')
  ...
  temp_hsr_dmg <- redistribute(subset_dmg, hsr_pc_map, source_id = "parid", target_id = 'geoid') 
  temp_pd_dmg <- redistribute(subset_dmg, pd_pc_map)
  temp_sd_dmg <- redistribute(subset_dmg, sd_pc_map)
  temp_zc_dmg <- redistribute(subset_dmg, zc_pc_map)
