...
get_2010_2020_bound_changes <- function(res = "tract", geoids = NULL) {
  ...
  # get crosswalk data for a given resolution
  if (res == "tract") {
    file_path <- "https://www2.census.gov/geo/docs/maps-data/data/rel2020/tract/tab20_tract20_tract10_natl.txt" # nolint: line_length_linter.
    res_code <- "TRACT"
    ...
    file_path <- "https://www2.census.gov/geo/docs/maps-data/data/rel2020/blkgrp/tab20_blkgrp20_blkgrp10_st51.txt" # nolint: line_length_linter.
    res_code <- "BLKGRP"
  ...
  crosswalk <- crosswalk %>% group_by(geoid20) %>% mutate(count_20 = n())
  crosswalk <- crosswalk %>% group_by(geoid10) %>% mutate(count_10 = n())

  # Identifying geoids that are same
  geoid_10_20 <- crosswalk %>%
    select(geoid10, area20) %>%
    group_by(geoid10) %>%
...
convert_2010_to_2020_bounds <- function(
  data,
  geoid_col = "geoid",
  val_col = "value"
) {
  ...
  if (length(data[, geoid_col]) > length(geoids)) {
    stop(
      "geoids are not unique -- 
      data cannot contain more than one entry per geoid. 
  ...
  if (sum(is.na(data[val_col])) > 0) {
    warning(
      "data contains missing or NULL values. the value of any new tract that 
  ...
  # case when tract borders don't change (same or split),
  # no more changes necessary
  same_bounds <- data.frame(joined)[
    joined$type_change == "same" | joined$type_change == "split",
  ] %>%
    group_by(geoid20) %>%
    ...
    if (yr < 2020) {
      for (meas in measures) {
        temp <- data.frame(data) %>%
          filter(year == yr, measure == meas, nchar(geoid) == 11)
