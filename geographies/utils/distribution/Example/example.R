...
standardized_data_std <- standardized_data %>%
  filter(measure == 'material_deprivation_indicator_std')
standardized_data_org <- standardized_data %>%
  filter(measure == 'material_deprivation_indicator_orig_2010')
...
my_plot <- ggplot(data = standardized_data_std) +
  ...
  theme_minimal()
ggsave(
  "utils/distribution/Example/standardized.png",
  plot = my_plot,
  width = 10,
  height = 8,
  dpi = 300
...
my_plot <- ggplot(data = standardized_data_org) +
  ...
  theme_minimal()
ggsave(
  "utils/distribution/Example/original.png",
  plot = my_plot,
  width = 10,
  height = 8,
  dpi = 300
